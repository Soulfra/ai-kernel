...
class TreeUnionFind {
    ...
    recurfind(x) {
        if (this.data[x][0] !== x) {
            this.data[x][0] = this.recurfind(this.data[x][0]);
    ...
    find(x) {
        var stack = [];
        let currentX = x;
        while (this.data[currentX][0] !== currentX) {
            stack.push(currentX);
            currentX = this.data[currentX][0];
        }
        const z = currentX;
        while (stack.length) {
            const currentX = stack.pop();
            this.data[currentX][0] = z;
            this.isComponent[currentX] = false;
        }
        return this.data[x][0];
    }
    components() {
