...
const ensureFileAndDir = require('../ensureFileAndDir');
...
describe('Kernel Slate: Backup+Buildup E2E', () => {
  ...
  beforeAll(() => {
    ...
    for (const relPath of FILES_TO_COPY) {
      ...
      const dest = path.resolve(tempDir, path.basename(relPath));
  ...
  it('creates a backup, wipes, builds up, and verifies integrity', async () => {
    ...
    for (const relPath of FILES_TO_COPY) {
      const file = path.join(tempDir, path.basename(relPath));
    ...
    for (const relPath of FILES_TO_COPY) {
      const orig = path.resolve(__dirname, relPath);
      const rebuilt = path.join(tempDir, path.basename(relPath));
